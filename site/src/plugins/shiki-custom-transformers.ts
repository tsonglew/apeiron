    ...
    pre(node) {
      const button = h(
        ...
        [
          h('div', { class: 'ready' }, [
            h('svg', { class: 'size-5' }, [
              h('use', { href: `${import.meta.env.BASE_URL}icons/code.svg#mingcute-clipboard-line` })
            ])
          ]),
          h('div', { class: 'success hidden' }, [
            h('svg', { class: 'size-5' }, [
              h('use', { href: `${import.meta.env.BASE_URL}icons/code.svg#mingcute-file-check-line` })
            ])
          ])
        ]
      )
  ...
  return {
    ...
    pre(node) {
      ...
      const collapse = h(
        ...
        [
          h('svg', { class: 'size-5' }, [
            h('use', { href: `${import.meta.env.BASE_URL}icons/code.svg#mingcute-arrow-down-line` })
          ]),
